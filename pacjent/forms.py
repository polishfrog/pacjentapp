from django import forms
from django.core.exceptions import ValidationError
from django.forms import ModelForm


def post_code_validation(post_code):
    if len(post_code) == 5:
        raise ValidationError("Kod pocztowy musi być w formacie 00-000")
    if post_code[2] is not '-':
        raise ValidationError("Kod pocztowy musi być w formacie 00-000")

def pesel_validation(pesel):
    if len(pesel) != 11:
        raise ValidationError("PESEL ma 11 liczb!")
    tab = (1, 3, 7, 9, 1, 3, 7, 9, 1, 3)
    suma = 0
    

"""   When admin or staff want add new user.    """
class NewUserForm(forms.Form):
    pesel = forms.CharField(max_length=11)
    first_name = forms.CharField(max_length=64)
    last_name = forms.CharField(max_length=64)
    password = forms.CharField(max_length=32, widget=forms.PasswordInput)
    password_repeat = forms.CharField(max_length=32, widget=forms.PasswordInput)
    date_of_birth = forms.DateField()
    street = forms.CharField(max_length=128)
    build_number = forms.CharField(max_length=8)
    apartment_number = forms.CharField(max_length=8)
    post_code = forms.CharField(max_length=6, validators=[post_code_validation])
    city = forms.CharField(max_length=128)
    mail = forms.EmailField()


class ChangeUserDataForm(forms.Form):
    street = forms.CharField(max_length=128)
    build_number = forms.CharField(max_length=8)
    apartment_number = forms.CharField(max_length=8)
    post_code = forms.CharField(max_length=6, validators=[post_code_validation])
    city = forms.CharField(max_length=128)


class LoginForm(forms.Form):
    pesel = forms.CharField(max_length=11)
    password = forms.CharField(max_length=32, widget=forms.PasswordInput)
